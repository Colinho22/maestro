...
def build_matrix(args: argparse.Namespace) -> list[dict]:
    ...
    # Validate strategy names up front (argparse no longer does, now that the
    # flag accepts a list). Catches a typo before any matrix work or API spend.
    if strategy_names:
        valid = {s.value for s in Strategy}
        unknown = [s for s in strategy_names if s not in valid]
        if unknown:
            print(
                f"ERROR: unknown --strategy value(s): {', '.join(unknown)}. "
    ...
    # the strategy filter.
    if model_names and real_strategies and not models:
